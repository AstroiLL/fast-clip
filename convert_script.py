            ...
            if len(cells) >= 8:
                try:
                    ...
                    timeline.append(TimelineItem(
                        ...
                        effect_during=parse_md_effect(cells[5]) if cells[5] else None,
                        end_effect=parse_md_effect(cells[6]),
                        end_duration=parse_md_duration(cells[7]),
                        description=cells[8] if len(cells) > 8 and cells[8] else None
                    ))
...
def convert_file(input_path: Path, output_path: Optional[Path] = None) -> Path:
    ...
    if input_path.suffix == '.md':
        ...
        raise ValueError(f"Unsupported file format: {input_path.suffix}. Use .md or .json")
