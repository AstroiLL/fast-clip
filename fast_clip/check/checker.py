            ...
            for i, item in enumerate(self.data["timeline"]):
                if isinstance(item, dict) and "resource" in item:
                    ...
                    if not resource_path.exists():
                        ...
                        self.add_result(f"Timeline[{i}].resource", "OK", f"Found: {resource}")
    ...
    def check_all(self) -> bool:
        ...
        for status, message, suggestion in validation_errors:
            ...
            if ":" in message:
                ...
                field = "Script"
            self.add_result(field, status, message.split(":", 1)[-1].strip() if ":" in message else message, suggestion)
...
def check_script(script_path: Path, verbose: bool = False) -> Tuple[bool, List[CheckResult]]:
