...
FORMAT_CODECS = {"mp4": "libx264", "avi": "mpeg4", "mov": "libx264", "mkv": "libx264"}
...
class ScriptConfig(BaseModel):
    ...
    output_format: Optional[str] = Field(
        None, description="Output format: mp4, avi, mov, mkv"
    )
    resolution: Optional[str] = Field(
        "1080p", description="Resolution: 2160p, 1440p, 1080p, 720p, 480p"
    )
    orientation: Optional[str] = Field(
        "landscape", description="Video orientation: landscape, portrait, square"
    )
    ...
    def get_output_format(self) -> Optional[str]:
        ...
        if fmt not in SUPPORTED_FORMATS:
            print(
                f"Warning: Unsupported output format '{self.output_format}'. Using source format."
            )
            return None
        return fmt
    ...
    def get_resolution(self) -> Optional[int]:
        ...
        if res not in RESOLUTIONS:
            print(
                f"Warning: Unsupported resolution '{self.resolution}'. Valid values: {', '.join(RESOLUTIONS.keys())}. Using source resolution."
            )
            return None
    ...
    def get_orientation(self) -> Optional[str]:
        ...
        if orient not in ORIENTATION_RATIOS:
            print(
                f"Warning: Unsupported orientation '{self.orientation}'. Will detect from first clip."
            )
            return None
        return orient
...
def calculate_target_size(
    resolution_height: Optional[int], orientation: Optional[str]
...
def fit_video_to_container(
    clip: VideoFileClip, container_size: Tuple[int, int]
) -> VideoClip:
    ...
    bg_clip = ColorClip(
        size=container_size, color=(0, 0, 0), duration=resized_clip.duration
    )
    ...
    final_clip = CompositeVideoClip(
        [bg_clip, resized_clip.with_position((x_center, y_center))]
    )
...
def load_and_process_clip(
    resources_dir: Path, item: TimelineItem, target_size: Optional[Tuple[int, int]]
...
def assemble_video(config: ScriptConfig, script_dir: Path) -> Path:
    ...
    if orientation is None:
        ...
        if first_video_path.exists():
            ...
            temp_clip.close()
            print(
                f"Detected orientation from first clip: {orientation} ({width}x{height})"
            )
        else:
            orientation = "landscape"
            print(
                f"Warning: Cannot detect orientation, first clip not found. Using landscape."
            )
...
def main():
    ...
    if not is_valid:
        ...
        print(f"For detailed info, run: python check_json.py -v {script_path}")
        sys.exit(1)
