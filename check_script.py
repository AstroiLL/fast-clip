...
class ScriptChecker:
    ...
    def add_result(self, field: str, status: str, message: str, suggestion: Optional[str] = None):
    ...
    def check_field_types(self):
        ...
        if "resources_dir" in self.data:
            if not isinstance(self.data["resources_dir"], str):
                ...
                self.add_result("Field 'resources_dir'", "OK", f"Value: '{self.data['resources_dir']}'")
        ...
        if "result_file" in self.data:
            if not isinstance(self.data["result_file"], str):
            ...
            elif not self.data["result_file"].endswith(('.mp4', '.avi', '.mov', '.mkv')):
                self.add_result(
                    "Field 'result_file'",
                    "WARNING",
                ...
                self.add_result("Field 'result_file'", "OK", f"Value: '{self.data['result_file']}'")
        ...
        if "timeline" in self.data:
            if not isinstance(self.data["timeline"], list):
                ...
                self.add_result("Field 'timeline'", "OK", f"{len(self.data['timeline'])} item(s)")
    ...
    def check_optional_fields(self):
        ...
        if "output_format" in self.data:
            ...
            if fmt is None:
                self.add_result("Field 'output_format'", "OK", "Not specified (will use source)")
        ...
        if "resolution" in self.data:
            ...
            if res is None:
                self.add_result("Field 'resolution'", "OK", "Not specified (will use source)")
        ...
        if "orientation" in self.data:
            ...
            if orient is None:
                self.add_result("Field 'orientation'", "OK", "Not specified (will detect from first clip)")
            ...
            if "resource" in item:
                ...
                if not isinstance(resource, str):
                    ...
                    if not resource_path.exists():
                        ...
                        self.add_result(f"{prefix}.resource", "OK", f"Found: {resource}")
                else:
                    self.add_result(f"{prefix}.resource", "OK", f"Value: '{resource}' (cannot verify - directory not found)")
            ...
            if "time_start" in item and "time_end" in item:
                try:
                    ...
                    if start >= end:
                        ...
                        duration = end - start
                        self.add_result(f"{prefix}.time", "OK", f"Duration: {duration}s")
                except:
    ...
    def _check_effect(self, prefix: str, item: dict, effect_field: str, duration_field: str):
        ...
        if duration is not None:
            if not isinstance(duration, str):
                ...
                try:
                    float(duration[:-1])
                    self.add_result(f"{prefix}.{duration_field}", "OK", f"Value: '{duration}'")
                except ValueError:
                    self.add_result(
                        f"{prefix}.{duration_field}",
                        "ERROR",
...
def main():
    ...
    for arg in sys.argv[1:]:
        if arg == "-v" or arg == "--verbose":
            ...
            print("Usage: python check_script.py [-v] <script.json>")
            sys.exit(2)
        else:
            script_path = arg
    ...
    if not script_path:
        ...
        print("")
        print("Examples:")
        print("  python check_script.py script.json          # Silent mode")
        print("  python check_script.py -v script.json       # Verbose mode")
        sys.exit(2)
